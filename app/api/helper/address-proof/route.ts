...
export async function POST(request: Request) {
  try {
    ...
    const objectKey = `private/address-proofs/${session.user_id}/${documentId}.${extension}`;
    ...
    await db.batch([
      db.prepare(
        `INSERT INTO helper_profiles (user_id, home_locality, verification_status, profile_status)
         VALUES (?, '', 'verified', 'draft')
         ON CONFLICT(user_id) DO UPDATE SET verification_status = 'verified', updated_at = CURRENT_TIMESTAMP`,
      ...
      db.prepare(
        `INSERT INTO verification_documents
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, 'verified')`,
      ).bind(documentId, session.user_id, documentType, objectKey, file.name.slice(0, 180), file.type, file.size),
      db.prepare(
        "INSERT INTO analytics_events (id, user_id, event_name, properties_json) VALUES (?, ?, 'address_proof_uploaded', ?)",
      ).bind(crypto.randomUUID(), session.user_id, JSON.stringify({ documentType, contentType: file.type, sizeBytes: file.size })),
    ]);
    await Promise.all(existing.results.map(item => bucket.delete(item.r2_object_key).catch(() => undefined)));
    return Response.json({ uploaded: true, filename: file.name, status: "provided" });
  } catch (error) {
